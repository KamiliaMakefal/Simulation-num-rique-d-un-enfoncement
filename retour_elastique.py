...
def fonction_generale(u_indentation):
        element_nbr=10
        element_nbr1=30
        Diametre= 80
        Epaisseur= 4
	...
	def create_assembly(model,part_name1,part_name2,instance_name1,instance_name2):
	    ...
	    a.translate(instanceList=(instance_name2, ), vector=(0.0,(Diametre/2)+20., 0.0))
	...
	def create_interaction(model,step_name,interaction_name,interaction_property):
	   ...
	   mdb.models[model].ContactStd(name=interaction_name, 
	...
	def Create_Load(model,step_name,surface_name):
	    ...
	    region = a.instances['cylindre-1'].surfaces[surface_name]
	    mdb.models[model].Pressure(name='Load-1', 
	    createStepName=step_name, region=region, distributionType=UNIFORM, field='', 
	    magnitude=mypenfoncement, amplitude=UNSET) 
	...
	Create_CL_sphere('Buckling_analysis',"p_enfoncement","BC_6",'RF_1',0.0,-(u_indentation+5),0.0,0.0,0.0,0.0,UNSET)
	mdb.models['Buckling_analysis'].boundaryConditions['BC_6'].setValuesInStep(stepName="p_desenfoncement", u2=30.0)    #desenfoncement
	Create_Load('Buckling_analysis','p_mise_en_pression','Surf-1')
	...
	List_seedEdgesbias = [[(Diametre/2),0.0,0.0,element_nbr,'non'], [(Diametre/2)-(Epaisseur),0.0,0.0,element_nbr,'non'], [(Diametre/2),0.0,10.0,element_nbr,'oui'], [(Diametre/2)-(Epaisseur),0.0,10.0,element_nbr,'non'],
		              [(Diametre/2),0.0,Longueur_Tube,element_nbr,'non'], [(Diametre/2)-(Epaisseur),0.0,Longueur_Tube,element_nbr,'oui'], [(Diametre/2)*cos(theta),(Diametre/2.)*sin(theta),Longueur_Tube/2.,element_nbr1,'oui'],         [0.0,(Diametre/2.),Longueur_Tube/2.,element_nbr1,'non'], [0.0,-(Diametre/2.),Longueur_Tube/2.,element_nbr1,'non'], [0.0,-((Diametre/2)-(Epaisseur)),Longueur_Tube/2.,element_nbr1,'non'], [0.0,((Diametre/2)-(Epaisseur)),Longueur_Tube/2.,element_nbr1,'non'], [0.0,((Diametre/2)-(Epaisseur)),Longueur_Tube/2.,element_nbr1,'non'], [((Diametre/2)-(Epaisseur))*cos(theta),((Diametre/2)-(Epaisseur))*sin(theta),Longueur_Tube/2.,element_nbr1,'non']]
	...
	Create_Job('enfoncement-4')
	#submit_job('enfoncement-4')
     
        #determination de Ureff(u apres le retour elastique)
...
u_num=0
u_reff= 4
u_indentation1 = u_reff *1.1
pas = u_indentation1/4
difference_u_indentation = ""

Compteur_calcul = 0
while abs(abs(u_num)-abs(u_reff))> 0.01 and (Compteur_calcul <10 ):
  Compteur_calcul += 1
  if abs(u_num) > u_reff:
     u_indentation1 = u_indentation1 - pas
     if difference_u_indentation == "augmente": pas /= 2
     difference_u_indentation = "diminue"
  else:
     u_indentation1 = u_indentation1 + pas
     if difference_u_indentation == "diminue": pas /= 2
     difference_u_indentation = "augmente"

  fonction_generale(u_indentation1)
  o1 = session.openOdb(
  name='/shared/home/6574MG/Desktop/kamilia/enfoncement-4.odb')
  session.viewports['Viewport: 1'].setValues(displayedObject=o1)
  odb = session.odbs['/shared/home/6574MG/Desktop/kamilia/enfoncement-4.odb']
  xyList = xyPlot.xyDataListFromField(odb=odb, outputPosition=NODAL, variable=(('U', NODAL, ((COMPONENT, 'U2'), )), ), nodeLabels=(('CYLINDRE-1', ('6', )), ))
  i = 0
  pas_temps = 0
  while pas_temps != 3:
    pas_temps = xyList[0][i][0]
    u_num = xyList[0][i][1]
    i += 1
     
  print(Compteur_calcul) 
  print(u_num)
  print(u_indentation1)
