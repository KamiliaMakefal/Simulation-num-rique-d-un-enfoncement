...
def fonction_generale(seedsize): 
	...
	def Power_law(Rp_, Rm_, E_Rm_ = 0.20, E_ = 210000.):
	    Power_law_ = []
	    #First point
	    Epsi_plastique_ = float(Rp_) / E_
	    Power_law_.append([Epsi_plastique_, Rp_])
	    ###Ludwik power law (JOHAN JANOT k and n)
	    #Sigma = Sigma_e + K * E^n
	    n_ = log(1 + (0.45 * log(Rm_/Rp_)))
	    k_ = (Rm_ * (1. + E_Rm_)) / (( log(1. + E_Rm_) )**n_)
	    #Make the def grid (0% a 8% de deformation)
	    Erp_ = pow((Rp_ / k_),(1.0 / n_))
	    for Val in range(1, 41):
		Sigm_ = Val * ((1.29*Rm_ - Rp_) /40. ) + Rp_
		Epsi_elas_ = Sigm_ / E_
		Epsi_plas_ = ( (Sigm_) / k_)**(1./n_) - Erp_
		Epsi_total_ = Epsi_elas_ + Epsi_plas_
		Power_law_.append([Epsi_total_, Sigm_])
	    return Power_law_

	Rp_ = 250  # Yield Strength (in MPa)
	Rm_ = 550  # Tensile Strength (in MPa)
	Power_law_data = Power_law(Rp_, Rm_, E_Rm_ = 0.20, E_ = 210000.)

	#Extract the data for plotting
	x = [i[0] for i in Power_law_data]
        x[0] = 0
	y = [i[1] for i in Power_law_data]
        z = list(zip(y, x))
        z1 = [tuple(element) for element in z]
	...
	def create_material_data(model,material_name,E,nu,z):
	     mdb.models[model].Material(name=material_name)
	     mdb.models[model].materials[material_name].Elastic(table=((E, nu), ))
            
	     mdb.models[model].materials[material_name].Plastic(table=z)
	...
	create_material_data('Buckling_analysis',"steel",210000.0,0.3,z1)
...
#lineeaire 
	...
	Create_Job('loi_de_comportement2')
	#submit_job()



fonction_generale(0.6)
