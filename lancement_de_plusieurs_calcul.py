...
def fonction_generale(Diametre,Epaisseur,mypenfoncement,mypmin,mypmax,seedsize): 
	...
	def create_analysis_step(model,step_name,min_inc,max_inc,pre_step_name):
	    ...
	    mdb.models[model].steps[step_name].setValues(maxNumInc=1000)
	...
	create_analysis_step('Buckling_analysis',"pmin",mymin_inc,mymax_inc,"p_desenfoncement")
	create_analysis_step('Buckling_analysis',"pmax",mymin_inc,mymax_inc,"pmin")
	create_analysis_step('Buckling_analysis',"peclat",mymin_inc,mymax_inc,"pmax")
	...
	Create_CL_sphere('Buckling_analysis',"p_enfoncement","BC_6",'RF_1',0.0,-10.0,0.0,0.0,0.0,0.0,UNSET)
	...
	#changement de pression/fatigue
	mdb.models['Buckling_analysis'].loads['Load-1'].setValuesInStep(stepName='pmax', magnitude=mypmax)
	mdb.models['Buckling_analysis'].loads['Load-1'].setValuesInStep(stepName='pmin', magnitude=mypmin) 
	mdb.models['Buckling_analysis'].loads['Load-1'].setValuesInStep(stepName="peclat", magnitude=17.5)
	...
	Create_Job('enfoncement-3')
	#submit_job()



with open("/shared/home/6574MG/Desktop/kamilia/Classeur1.csv",'r') as file:
     myfile = csv.reader(file,  delimiter=";")
     for ligne in myfile:
         for num, val in enumerate(ligne) : ligne[num] = str(val).replace(",",".")
         m1=str(ligne).replace("'","")
         m2=str(m1).replace("[","")
         m3=str(m2).replace("]","")
         arr=m3.split(",")
         fonction_generale(float(arr[0]),float(arr[1]),float(arr[2]),float(arr[3]),float(arr[4]),float(arr[5]))
